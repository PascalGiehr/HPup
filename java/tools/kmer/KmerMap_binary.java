...
public class KmerMap_binary extends HashMap<BitSet, Integer> implements Serializable{

	private static final long serialVersionUID = 1L;
	private int kmerSize;
	private KmerSet_binary_utils kt_utils;
	private HashMap<BitSet, Integer> Buffer;
	private final int bufferSize=20000000;
	
	public KmerMap_binary(int kmerSize){
		super(600000000);
		this.kmerSize=kmerSize;
		kt_utils= new KmerSet_binary_utils(kmerSize);
		Buffer= new HashMap<BitSet, Integer>(bufferSize*2);
	}
...
//			System.err.println(kt_utils.bitSetToString(kt_utils.kmerToUse(kmer))+" O");
	...
	protected void addBuffer(HashMap<BitSet, Integer> m2){
		Iterator<Entry<BitSet, Integer>> it=m2.entrySet().iterator();
		while(it.hasNext()){
	...
	public boolean addKmer(String kmer,Integer count){
	...
	public Integer get(String kmer_string){
	...
	public Integer get(StringBuffer kmer_stringBuffer){
		return this.get(kmer_stringBuffer.toString());
	}
	
	public boolean addKmer(BitSet kmer,Integer count){
		if(this.containsKey(kmer)){
	...
	public void print(){
		...
		System.err.println(" Done");*/
		Iterator<Entry<BitSet, Integer>> it = this.entrySet().iterator();
		while(it.hasNext()){
			Entry<BitSet, Integer> e= it.next();
